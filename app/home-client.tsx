      ...
      <section className="interactive-hero" aria-labelledby="site-title">
        <div className="scene-canvas">
          ...
          <header className="hero-copy">
            <p className="eyebrow">非遗数字交互装置 · DIGITAL INSTALLATION</p>
            ...
            <p className="intro">在身体参与中感受民艺的生命力，<br />让延续百年的造物智慧在数字语境中重新生长。</p>
          ...
          <img className="sugar-rack-back" src={media("sugar-rack-back-v4.png")} alt="" aria-hidden="true" />
          ...
          <div className="sugar-stage" aria-label="选择一支糖人查看项目内容">
            {stories.map((story) => (
              ...
              >
                ...
                <span className="sugar-label">
                  <small>{story.number}</small>
                  <span className="sugar-label-title" aria-hidden="true">
                    {[...story.title].map((character, index) => <span key={`${character}-${index}`}>{character}</span>)}
                  </span>
                </span>
              </button>
            ))}
          ...
          <img className="sugar-rack-front" src={media("sugar-rack-front-v4.png")} alt="" aria-hidden="true" />
        </div>
      </section>
